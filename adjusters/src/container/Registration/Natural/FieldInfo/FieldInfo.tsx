...
const FieldInfo: FC<IFieldInfoProps> = ({ onSubmit, onPrev }) => {
  ...
  // const {noInquireCertificatePic}=useSelector(
  //   (state: any) => state?.noInquireCertificatePic
  // );

  //console.log(noInquireCertificatePic,"noInquireCertificatePic");
  ...
  useEffect(() => {
    if (!isUserEdit) {
      ...
      if (adjustmentFiledInfo?.Course80HourDocumentId !== null) {
        if (adjustmentFiledInfo?.IsExamNotNeeded === true) {
          setIsAllFieldsAndSubFieldsInquired(true);
        } else if (
