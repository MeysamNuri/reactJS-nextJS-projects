...
const Filter: FC<IFilterProps> = ({
  ...
  const [filterdCartable, setFilterdCartable] = useState<any>([]);
  ...
  const { modelFilterNaturalCartable } = useSelector(
    (state: any) => state.listNaturalCartable
  );
  ...
  const onFinish = (values: any) => {
    ...
    if (values.StateId != undefined && values.StateId.length !== 0) {
    ...
    if (values["Course.Id"] != undefined && values["Course.Id"].length !== 0) {
    ...
    if (
      values.AdjustmentFieldId !== undefined &&
      values.AdjustmentFieldId.length !== 0
    ) {
  ...
  // useEffect(() => {
  //  let test= modelFilterNaturalCartable?.advancedSearchModel?.filters.filter((item:any)=>item.propertyName=="StateId").map((item:any)=>item.value)
  //  return test

  // }, [activeTabInbox,modelFilterNaturalCartable,test])

  useEffect(() => {
    form.setFieldsValue({
      NationalCode: modelFilterNaturalCartable?.advancedSearchModel?.filters.find(
        (item: any) => item.propertyName == "NationalCode"
      )?.value,
      FirstName: modelFilterNaturalCartable?.advancedSearchModel?.filters.find(
        (item: any) => item.propertyName == "FirstName"
      )?.value,
      FamilyName: modelFilterNaturalCartable?.advancedSearchModel?.filters.find(
        (item: any) => item.propertyName == "FamilyName"
      )?.value,
      RegistrationCode: modelFilterNaturalCartable?.advancedSearchModel?.filters.find(
        (item: any) => item.propertyName == "RegistrationCode"
      )?.value,
      "Course.Id": modelFilterNaturalCartable?.advancedSearchModel?.filters
        .filter((item: any) => item.propertyName == "Course.Id")
        .map((item: any) => item.value),
      StateId: modelFilterNaturalCartable?.advancedSearchModel?.filters
        .filter((item: any) => item.propertyName == "StateId")
        .map((item: any) => item.value),
      AdjustmentFieldId: modelFilterNaturalCartable?.advancedSearchModel?.filters
        .filter((item: any) => item.propertyName == "AdjustmentFieldId")
        .map((item: any) => item.value),
      SubFields: modelFilterNaturalCartable?.advancedSearchModel?.filters.find(
        (item: any) => item.propertyName == "SubFieldId"
      )?.value,
    });
  }, [activeTabInbox, modelFilterNaturalCartable]);
