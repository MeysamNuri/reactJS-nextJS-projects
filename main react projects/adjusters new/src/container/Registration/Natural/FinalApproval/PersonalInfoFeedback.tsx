...
const PersonalInfoFeedback: FC<IPersonalInfoFeedbackProps> = ({
  ...
  let religions = base?.baseInfo?.Result?.Religions;

  let resultBaseInfo = base?.baseInfo?.Result;
  ...
  let findResidenceCity = findResidenceCityObject?.Cities?.find(
    ({ Id, Title }: { Id: number; Title: string }) => personalInfo.CityId === Id
  );
      ...
      <ConfigProvider direction="rtl">
        ...
        <Row>
          <Col md={11} offset={1}>
            ...
            <Form.Item label="نام" style={{ textAlign: "right" }}>
              <span>{personalInfo?.Person?.FirstName}</span>
            </Form.Item>
            <Form.Item label="نام خانوادگی" style={{ textAlign: "right" }}>
