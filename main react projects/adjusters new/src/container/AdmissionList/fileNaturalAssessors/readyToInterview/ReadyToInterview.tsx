...
import DateInterviewAndInterviewers from "./DateInterviewAndInterviewer";
...
// import ReadytoInterview from "../../assets/images/readytoInterview.png";
import { ReactComponent as Xls } from "../../../../assets/images/xls.svg";
// import ReadytoInterview from "../../assets/images/readytoInterview.png";
// import AddUser from "../../assets/images/calendar.svg";


interface IReadyToInterviewProps {
  handleChildSelectData: any;
  activeTab: string;
  handleChildData: any;
}
const ReadyToInterview:FC<IReadyToInterviewProps>=({handleChildSelectData,handleChildData}) => {
  ...
  const [cartableId, setCartableId] = useState(0);
  const { allCourseByAdjusterType } = useSelector(
    (state: any) => state?.allCourse
  );
  const { loadingInvitionInterview } = useSelector((state: any) => state.score);
  const { loading: loadingInformationFile } = useSelector(
          ...
          ? [
              {
                ...
                value: workTaskFlowId.ReadyToInterview,
              },
            ]
          : filterList,
    };
  ...
  useEffect(() => {
    dispatch(fetchAllCourseByAdjusterType(adjusterType.natural));
  }, []);
  ...
  const interviewInvitationHandler = (record: any) => {
  ...
  const addTimeInterviewHandler = (record: any) => {
    setApplicantId(record.ApplicantId);
    setCartableId(record.CartableId);
    setVisibleTime(true);
  };
  ...
  const columns = [
...

    {
      ...
      render: (p: any, record: any) => (
        <>
          {/* <Tooltip title="زمان مصاحبه/مصاحبه کنندگان" placement="topLeft"> */}
          <Button
            type="primary"
            onClick={() => addTimeInterviewHandler(record)}
            size="small"
            style={{ fontSize: "12px" }}
          >
            زمان مصاحبه/ مصاحبه کننده
          </Button>
          {/* </Tooltip> */}
          {/* <Tooltip title="دعوت به مصاحبه" placement="topLeft"> */}
          <Button
            type="primary"
            onClick={() => interviewInvitationHandler(record)}
            size="small"
            style={{ fontSize: "12px" }}
          ...
          <Button
            type="primary"
            onClick={() => downloadInformationFile(record)}
            size="small"
            style={{ fontSize: "12px" }}
            loading={loadingInformationFile === record.ApplicantId}
          >
            اطلاعات پرونده
          </Button>
  ...
  const downloadInformationFile = (record: any) => {
    dispatch(informationFileAction(adjusterType.natural, record.ApplicantId));
  };
      ...
      <span>انتخاب شده ها {selectedDocumentList.length}</span>
        ...
        >
          <DateInterviewAndInterviewers
            closeModal={() => setVisibleTime(false)}
            isCartable={false}
