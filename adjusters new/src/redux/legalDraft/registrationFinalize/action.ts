...
export const registerationFinalizeLegal = (
  draftId: number,
  ...
  try {
    dispatch({
      type: REGISTRATION_FINALIZE_LEGAL,
    });
    loadingFunction();
    ...
    dispatch({
      type: REGISTRATION_FINALIZE_LEGAL_SUCCESS,
      payload: data,
    });
  } catch (error: any) {
    dispatch({
      type: REGISTRATION_FINALIZE_LEGAL_FAILED,
      payload:
