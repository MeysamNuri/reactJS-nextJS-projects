...
const InterviewInvitation: FC<IInterviewProps> = ({ handleChildSelectData, handleChildData }) => {
    ...
    if ((dataIndex == "InterviewTime")) {
      ...
      if (findedItem) {
        ...
        newFilters.push({
          ...
          value: workTaskFlowId.InterviewInvitation,
        });
  ...
  // //مدال نمره
  // const submitScoreHandler = (record: any) => {
  //   setRecord(record);
  //   console.log(record, "recordddds");

  //   setVisibleScore(true);
  //   dispatch(fetchInterviewerScore(record.ApplicantId));
  // };

  // //نشان دادن مدال رمینه تخصصی
  // const fieldHandler = (record:any) => {
  //   setRecord(record)
  //   setFields(true);
  // };
  ...
  const columns = [
...

    {
      ...
      render: (p: any, record: any) => (
        <>
          {/* <Tooltip title="ثبت نمره" placement="topLeft">
            <Button
              type="text"
              onClick={() => submitScoreHandler(record)}
              icon={<ScoreBoard />}
            ></Button>
          </Tooltip>
          <Tooltip title="ثبت رشته" placement="topLeft">
            <Button type="text" onClick={()=>fieldHandler(record)} icon={<Node />}></Button>
          </Tooltip> */}
  ...
  return (
    <div>
      {/* <div className="removeFilters">
        <Button onClick={handleReset}>پاکسازی همه فیلتر ها</Button>
      </div> */}
      ...
      <ConfigProvider direction="rtl">
        ...
        {/* <Modal
          title={`ثبت نمره ${record?.FirstName} ${record?.FamilyName}`}
