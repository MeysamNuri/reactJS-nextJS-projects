...
const Legal: FC<ILegalProps> = ({
  ...
  const { comnayTypesInfo } = useSelector(
  ...
  let companyTypeFilters = comnayTypesInfo?.Result?.map((item: any) => {
    let provinceObj = {
      key: item.Value,
  ...
  let data = reportLegalEvalutor?.Result?.map((report: IReport) => {
    let obj = {
      ...
      CompanyTypeDescription: report.CompanyTypeDescription,
  ...
  const handleChange = (pagination: any, filters: any, sorter: any) => {
    ...
    if (filters.CompanyTypeDescription != null) {
      let first = [...filters.CompanyTypeDescription].shift();
      let last = [...filters.CompanyTypeDescription].pop();
      let fitrstIndex = filters.CompanyTypeDescription.indexOf(first);
      let lastIndex = filters.CompanyTypeDescription.indexOf(last);
      setCityFilters(filters?.CompanyTypeDescription)
      if (fitrstIndex === lastIndex) {
        filteredIndictmentList.push({
          propertyName: "CompanyType",
          operator: 1,
          value: filters?.CompanyTypeDescription[0],
        });
      }

      if (filters.CompanyTypeDescription.length > 1) {
        for (let i = 0; i < filters.CompanyTypeDescription.length; i++) {
          //اولی
          if (i == fitrstIndex) {
            filteredIndictmentList.push({
              propertyName: "CompanyType",
          ...
          //آخری
          if (i == filters.CompanyTypeDescription.length - 1) {
            filteredIndictmentList.push({
              propertyName: "CompanyType",
              operator: 1,
              value: last,
          ...
          //منهای اخری
          if (i !== filters.CompanyTypeDescription.length - 1 && i !== fitrstIndex) {
            filteredIndictmentList.push({
              propertyName: "CompanyType",
              operator: 1,
              value: filters?.CompanyTypeDescription[i],
              operand: 1,
              isOpenGroup: false,
              isCloseGroup: false,
            });
